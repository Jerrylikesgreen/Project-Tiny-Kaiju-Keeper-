...
# Runs only once when the node is first instanced
func _ready() -> void:
	if not is_connected("body_entered", _on_body_entered):
		body_entered.connect(_on_body_entered)

# Called by the spawner every time the pool re‑uses this hazard
...
func _on_body_entered(_body: Node2D) -> void:
	Events.mini_game_hazard_trigger("Cookie")
	_deactivate()
